...
const DEFAULT_ERROR_HANDLER = async <State>(context: Context<State>) => {
  const { error } = context;
...
export class App<State> {
  ...
  notFound(routeOrMiddleware: Route<State> | MiddlewareFn<State>): this {
    const route = typeof routeOrMiddleware === "function"
      ...
      : routeOrMiddleware;
    ensureHandler(route);
    this.#root.notFound = (context) => renderRoute(context, route);
  ...
  route(path: string, route: Route<State>): this {
    ...
    ensureHandler(route);
    middlewares.push((context) => renderRoute(context, route));
  ...
  handler(): (
    ...
    return async (
      ...
      try {
        ...
        const result = await runMiddlewares(handlers, context);
        ...
        context.error = err;
        return await DEFAULT_ERROR_HANDLER(context);
      }
    };
  }
